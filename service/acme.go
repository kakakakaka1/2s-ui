...
func (a *AcmeService) IssueWeb(domain, email, method string, force bool) (*IssueResult, error) {
	if runtime.GOOS == "windows" {
	...
	// 仅 nginx 验证配 reloadcmd(reload nginx 让其重读证书)。standalone 不配:
	...
	if rc := a.buildReloadCmd(resolved); rc != "" {
...
// buildReloadCmd 决定续期成功后的重载命令:nginx 验证时 reload nginx(不影响面板进程);
// standalone 返回空——面板/订阅证书由 certReloader 热加载,无需外部命令(见 IssueWeb 注释)。
func (a *AcmeService) buildReloadCmd(method string) string {
	if method == methodNginx {
