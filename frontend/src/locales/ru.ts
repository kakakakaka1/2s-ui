...
export default {
  ...
  node: {
    ...
    importOk: "Входящие импортированы",
    reconcile: "Синхронизировать",
    syncOk: "Синхронизация запущена",
  ...
  setting: {
    ...
    behindProxyHint: "Панель остаётся на HTTP; выпущенные сертификаты предназначены для прокси",
    ...
    certForProxy: "Сертификат: {cert}, ключ: {key}. Укажите эти файлы в обратном прокси; панель остаётся на HTTP.",
    ...
    issueCertRestarting: "Сертификат готов. Панель перезапускается и переходит на HTTPS…",
    ...
    webUri: "URI панели",
    webUriProxyHint: "Обязательно за прокси: панель не может определить свой внешний адрес, и переход после перезапуска зависит от этого поля",
